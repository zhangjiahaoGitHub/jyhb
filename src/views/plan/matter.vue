<template>
  <div class='hundred matter-layout'>
    ...
    <div v-for='item in aisleList' :key='item.kstatus' class='planContent exhaustiveContent'>
          <ul>
            ...
            <li class='allFlex justifyBetween flexPadding'>
              <div class='redColor'>查看通道规则</div>
              <div>
                <a class='matterSubmit' :class="item.status == '未开通' ? '' : 'eeeBack'" @click="item.status == '未开通' ? toNext(1, item.code, item.category) : toNext(2, item.code, item.category) ">
                    {{item.status}}
                </a>
              </div>
            </li>
